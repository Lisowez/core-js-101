...
function getDigitalRoot(num) {
  let sum = num.toString().split('').map((item) => +item).reduce((x, y) => x + y, 0);
  if (sum > 9) {
    sum = sum.toString().split('').map((item) => +item).reduce((x, y) => x + y, 0);
  }
  return sum;
}
